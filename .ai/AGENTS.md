...
- **Comments Carry Only What Code Can't Say**: Delete a comment that restates the code. A previous or deleted implementation belongs in git — not in a comment, and not in a decision note or doc either. Keep what a reader can't derive: why a line exists (`// for idempotency`), why a workaround is there and when it can go, one real example value for a marker or format, and an input→output example on a parsing or derivation method (`// e.g. (in) "limit_32" → (out) 32`). A why-comment states the real reason; when you don't know it, state the rule plainly instead of inventing a justification. Mark a deliberately temporary state "for now" and name the risk accepted. One line where one line covers it. A user-facing "unavailable" message follows the same rule: say what isn't supported, not the mechanism behind it.
