...
- **Comments Stand Alone**: A code comment, PR body, or issue description must make sense to a future reader with no access to this chat. Don't leave notes that only parse with the conversation's context ("as discussed", "the value we picked above"); state the durable why, or drop the comment.
- **Comments Carry Only What Code Can't Say**: Delete a comment that restates the code. A previous or deleted implementation belongs in git — not in a comment, and not in a decision note or doc either. Keep what a reader can't derive: why a line exists (`// for idempotency`), why a workaround is there and when it can go, the external source a block mirrors (the sibling repo, package, or spec section it was ported from, with the spec's field names on each item of a positional structure), one real example value for a marker or format, and an input→output example on a parsing or derivation method (`// e.g. (in) "limit_32" → (out) 32`). A why-comment states the real reason; when you don't know it, state the rule plainly instead of inventing a justification. Mark a deliberately temporary state "for now" and name the risk accepted. One line where one line covers it, but a multi-step flow lists one step per line rather than a wrapped paragraph: readability beats line count. A user-facing "unavailable" message follows the same rule: say what isn't supported, not the mechanism behind it.
...
- **Separate Pre-Existing From Introduced**: When you surface a defect while reviewing or building, say whether the change in flight introduced it or it was already there. A pre-existing issue belongs in its own follow-up change — don't fold drive-by fixes into the current branch. Renaming an existing identifier or dropping an existing comment outside the change's purpose is a drive-by too; leave both as they were. Keep each change small enough that a reviewer can take it in without a big context switch, and split larger work into sequenced changes rather than one wide one. When those sequenced changes become dependent PRs, invoke the `gh-stack` skill and stack them with `gh stack` — never chain PR bases by hand.
...
- **A Swap Keeps The Output**: When replacing a library or implementation behind an existing output (a PDF renderer, an export format, a serializer), the result must match the old one visually and structurally. A cosmetic difference is a defect to fix before presenting the work, not a note to leave in the PR.
...
- **Merge Helpers Always Called Together**: When two complementary helpers are only ever called as a pair from the same site, make them one function returning both results (`{ available, unavailable }`) instead of two calls the reader has to correlate.
- **Fix Dependencies At The Root**: For a vulnerable or stale dependency, first check whether the code still uses it and remove it if not; otherwise bump the direct dependency. A package-manager override or pin is a last resort for a transitive dependency the direct one can't move yet, carries a comment naming when it can go, and is deleted once that reason is gone.
- **Name Reflects Behavior, Not Assumption**: When a method or variable's real behavior doesn't match what its name implies — a qualifier it doesn't actually check, a filter it silently applies that the name doesn't mention, a descriptor that overstates precision (`getLatestX` with no such ordering) — rename it to match the real behavior instead of leaving the mismatch for the next reader to trip over. Name complementary helpers as an antonym pair (`getAvailableX` / `getUnavailableX`), not two unrelated synonyms.
