...
6. **Log it.** Add one line to the **top** of `log.md` (newest first, so the latest updates are visible without scrolling): `## [YYYY-MM-DD] ingest | <what changed; link the pages>` (use today's date).
