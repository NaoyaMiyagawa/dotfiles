...
4. **Framework helpers over hand-rolled primitives.** `Illuminate\Filesystem\Filesystem` (`ensureDirectoryExists()`, `deleteDirectory()`), `Uri`, `Str`/`Arr`, collection helpers — not manual `scandir`/string/array fiddling. Applies in test setup/teardown too. And when the repo wraps a framework class in its own helper (a storage/file wrapper over the `Storage` facade, say), use the wrapper — reach for the bare facade only where no wrapper exists.
...
8. **Exceptions: named constructors + boundary handling.** `throw DomainException::invalidRequest()` — the exception class owns message keys, status codes, and any error-code string it matches on (a class constant such as `ERROR_CODE`, never a literal at the call site); catch expected domain failures at the controller/edge and map them to the spec'd error response instead of leaking a 500. Scope a `try`/`catch` to the statement that can actually throw — statements it doesn't protect sit outside the block.
...
1. Review the diff — only the diff — against the Core rules above and [references/checklist.md](references/checklist.md); for test files, also the `custom-laravel-writing-tests` skill's core rules and its `references/checklist.md`.
