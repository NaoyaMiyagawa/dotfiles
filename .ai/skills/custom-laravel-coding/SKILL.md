...
16. **A comment carries only what the code can't say.** Delete one that restates the code. The previous implementation belongs in git, a deferred design in the decision doc (core rule 5) — neither belongs in a comment. Keep what a reader can't derive: why a line exists at all (`// for idempotency`), why a workaround is there and when it can go. Mark a deliberately temporary state "for now" and name the risk accepted until it changes. One line where one line covers it.
