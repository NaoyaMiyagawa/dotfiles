...
6. Use named args when method calls goes multiple lines due to line length, and when calling into a package or other external API — the parameter order there can be re-ordered out from under you, and named args turn that into a compile-time break instead of silently mismatched arguments.
...
12. Use one term and one spelling per concept. Prefer American English (e.g. `organization`, not `organisation`); don't mix `-ize`/`-ise`. Where a feature has a user-facing name, that name is the term for its identifiers, routes, and translation keys — a synonym left behind (a `clone_failed` key under a "Duplicate" feature) reads as a second concept. Comments are exempt. Preserve the conventional casing of acronyms and mixed-case terms in identifiers (`OAuth`, `ID`, `URL`, `HTTP` — not `Oauth`/`Id`/`Url`).
...
- **Defer speculative flexibility; log the deferred decision instead of coding it.** When a requirement doesn't exist yet (multi-tenant, format-agnostic, pluggable), build the simple single-case version now and record the deferred design — trigger and intended approach — in the project's decision doc, rather than encoding an unused abstraction layer. Capture _why and when_, not _what the code does_. This holds for individual members too: don't land an interface method, enum case, or helper whose only consumer is a planned follow-up change — add it in the change that uses it. And skip a predicate that only wraps a trivial check on an already-public accessor (`hasItems()` over `empty($x->items())`); the call site can do that itself.
- **Collapse two types in the same change that model the same thing.** When a change introduces a pair of classes whose fields and purpose largely overlap, merge them before landing rather than keeping both because each grew from a different call path.
...
- **Express the constraint as a rule; don't hand-roll it.** Before writing an `if` check, a type check, or a normalisation step around validated input, look for the built-in rule that already covers it — `json` instead of decoding by hand, `min:1` instead of a whitespace-only check, `bail` on a rule set instead of a follow-up `if` that short-circuits. Also don't re-check what validation already guarantees downstream, and remember the framework trims request strings for you. Every hand-rolled equivalent is a second place the contract lives.
...
3. **Name a class for its real conceptual boundary, not a convenient umbrella prefix.** Don't stamp one feature's prefix onto components that actually belong to a distinct concept it merely sits on top of — when A is built on B, a piece that is really part of B is named for B. Separating the names keeps the concepts from blurring together.
4. **Keep response/output shaping out of action/service/domain classes.** A class that performs an operation shouldn't also assemble the HTTP response payload. When it does both, extract a dedicated response class so each has a single responsibility.
5. **Extract shared/cross-cutting logic into the repo's designated helper location** rather than duplicating it across controllers. Reuse the canonical helper; don't reinvent the same snippet per call site.
6. **Match the declared namespace to the file's directory.** When a change adds several files, check all of them, not just the one under discussion — a mismatch doesn't fail until something tries to autoload the class.
...
4. **Add a summary PHPDoc to a method or class whose purpose isn't obvious from its name.** For domain- or protocol-specific logic, an unfamiliar algorithm, or a non-trivial transformation, a one-line docblock stating _what it is / why it exists_ orients the next reader who lacks that context. Skip it when the name already says everything — a docblock that only restates the signature is noise.
5. **Link the external spec when implementing a standard, uniformly.** When code implements a published spec/RFC/protocol, put a link to the exact relevant section in the docblock — and do it for _every_ related endpoint/class, not just the first one, so the whole surface is traceable back to the source.
...
3. **Column order: foreign keys right after the `id`/`uuid` key columns.** The exception is audit-style FKs (`created_by`, `updated_by`) — group those near the `timestamps()` instead, since they belong to the record's bookkeeping rather than its relationships.
...
5. **Name a controller for the specific operation it performs, not a generic resource.** A controller that resends an invoice email is `ResendInvoiceEmailController`, not a catch-all `InvoiceController`; the class name should tell a reader what the endpoint does. For a single-action controller, use **verb + resource** — `DuplicateEmailTemplateController`, not the nominalised `EmailDuplicationController`. Keep the route, controller, directory, and test file names in sync (rule 2).
6. **Match the action method to the endpoint's shape.** When an endpoint maps to a CRUD operation, use the resourceful method name (`index`, `show`, `store`, `update`, `destroy`) on the controller rather than `__invoke`. Reserve a single-action `__invoke` controller for operations that are genuinely not resourceful. Don't default every controller to `__invoke`. When you do register a single-action controller, use the array-callable form `[Controller::class, '__invoke']` over the bare `Controller::class` string — the explicit method reference gives better IDE navigation.
...
1. **URL config values**: when an env holds a base URL (e.g. `https://cdn.example.com`), name the config key with a `_base_url` suffix (`cdn_base_url`, not `cdn_bucket`) and strip trailing slashes at read time: `rtrim(env('CDN_BASE_URL', ''), '/')`. Callers should not have to defend against `//path` joins.
