...
3. **Reuse domain language.** Before writing a raw conditional on a domain concept (role, membership, status, capability), grep the relevant model for an existing predicate/accessor — `isXxx()`, `hasXxx()` — and use it:
...
4. **Framework helpers over hand-rolled primitives.** `Illuminate\Filesystem\Filesystem` (`ensureDirectoryExists()`, `deleteDirectory()`), `Str`/`Arr`, collection helpers — not manual `scandir`/string/array fiddling. Applies in test setup/teardown too.
