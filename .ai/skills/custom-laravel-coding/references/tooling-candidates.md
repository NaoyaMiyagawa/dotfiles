...
| Request URLs in tests built with `route()`, never a path literal (writing-tests skill, Feature Test Pattern #1) | custom PHPStan rule over test files: flag a string literal starting with `/` as the first arg of `get()`/`post()`/`postJson()`/… |
| No raw `*_id` key in a factory attribute array — use a `forXxx()` state (writing-tests skill, Factory) | custom PHPStan rule over test files: flag array keys ending in `_id` passed to `::factory()`/`->create*()`/`->state()` |
| One chained call per line in a request chain — no `])->assertXxx()` on the closing line (writing-tests skill, Feature Test Pattern #2) | Pint/php-cs-fixer `method_chaining_indentation` plus a custom check that a `->assert*(` call follows a newline |
| Test file basename maps 1:1 to a production class (`XxxTest.php` ↔ class `Xxx`) (writing-tests skill, Test file naming) | small script in CI: for each `*Test.php`, assert a class with the stripped name exists in the autoload map |
