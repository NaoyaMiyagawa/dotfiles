...
| Model class, not a table-name string, as the first arg of every `assertDatabase*()` (writing-tests skill, core rule *Feature request shape*) | custom PHPStan rule over test files: flag a string literal as the first arg of `assertDatabaseHas/Missing/Count/Empty` |
| Break after `::query()` when the chain spans lines (checklist, *Eloquent*) | Pint/php-cs-fixer `method_chaining_indentation` plus a custom check that a `::query()` followed by `->` on the same line has no further `->` on later lines |
